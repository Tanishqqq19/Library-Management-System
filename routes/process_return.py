    ...
    def process_return(book_id):
        ...
        try:
            with sql.connect('library.db') as conn:
                ...
                records_query = ("SELECT records_id FROM records "
                ...
                for record_id, in records:
                    update_query = ("UPDATE records SET from_date = ?, to_date = ?, "
                                    "book_returned = 'Yes' "
                                    "WHERE borrow_user_id = ? AND book_returned = 'No' "
                                    "AND records_id = ?;")
                    cur.execute(update_query, (hundred_days_ago, hundred_days_ago, user_id, record_id))
                    break 

                book_details_query = ("SELECT books_name, author, image FROM books "
                                    "WHERE book_id = ?;")
        ...
        return render_template('returning_page.html', ret="Return was successful", 

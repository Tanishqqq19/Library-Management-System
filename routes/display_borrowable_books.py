        ...
        try:
            with sql.connect('library.db') as connection:
                ...
                for book_id in available_book_ids:
                    cursor.execute('SELECT books_name FROM books WHERE book_id=?', [book_id])
                    book_name = cursor.fetchall()
                    book_names.append(book_name)

                    cursor.execute('SELECT image, books_name, author FROM books WHERE book_id=?', [book_id])
            ...
            return render_template('display_borrowable_books.html', book_names=book_names, book_full_details=book_full_details)
